...
任务: 根据【用户偏好输入】与【工具检索结果】进行候选集融合、去重、评分与排序, 输出最多5个(1~5个)最匹配的餐厅。只保留真正合适的, 宁缺毋滥; 绝不为凑满5个而加入不相关、明显不符或与指定地点相距很远(不同城市/国家)的餐厅。
...
3) 约束过滤 (硬条件优先):
   ...
   - 地点: 所有结果必须靠近【用户偏好输入】中 Location 指定的地点, 并与该地点处于同一城市/国家/区域; 优先同商圈, 不足时可放宽到邻近商圈, 但绝不跨到不同城市/国家。严禁返回与指定地点相距很远(如另一个国家, 例如同名地名解析到了美国 Chinatown)的餐厅, 哪怕因此结果少于5个。请在结果中标注距离/通达性。
  ...
  "recommendations": [
    {
      ...
      "address": "...",
      "area": "...",
      "gps_coordinates": {"latitude": 1.2345, "longitude": 103.8765},
...
- 返回 1~5 条真正合适的结果 (按匹配度排序); 宁可少于5条, 也绝不为凑数加入不合适、明显不符或与指定地点相距很远(不同城市/国家)的项。候选不足时直接返回现有条目, 并在 why 中说明样本不足。
- gps_coordinates 必须直接取自工具检索结果(如 Google Maps)中的坐标, 不可编造; 若工具结果没有坐标则省略该字段。
- 字段缺失用 null 或省略, 不可捏造。
- 绝不输出除 JSON 以外的任何文本。"""
