...
JSON格式:
{{"intent":"confirmation_yes|confirmation_no|query|chat", "reply":"回复", "confidence":0.0-1.0, "preferences":{{"domain":"restaurant|movie|music|book|product", "query":"用户原始请求", "genres":["science fiction"], "mood":"relaxing", "tags":["award-winning"], "actors":["Cillian Murphy"], "directors":["Christopher Nolan"], "artist":"Daft Punk", "author":"Brandon Sanderson", "publisher":"Tor", "product":"iPhone", "category":"smartphone", "brand":"Apple", "model":"iPhone 14-16", "use_case":"iOS testing", "budget":"<= 1600 SGD", "restaurant_types":["casual"], "flavor_profiles":["spicy"], "dining_purpose":"friends", "budget_range":{{"min":20,"max":60,"currency":"SGD","per":"person"}}, "location":"Chinatown", "food_intent":{{"cuisines":["vietnamese"]或[], "dishes":["pho"]或[], "confidence":0.0-1.0}}}}, "profile_updates":{{"demographics":{{}}, "dining_habits":{{}}}}}}

规则: 只有在用户明确提出推荐/查找/修改推荐条件时才用"query"; 普通闲聊/问候/感谢一律用"chat"; 只填写用户明确表达或上下文强支持的preferences; 非餐厅请求不要填餐厅默认值; "confirmation_yes"和"chat"时preferences为null; profile_updates可选,仅推断新信息时提供,严格遵循字段规则; 当intent为"chat"时先正常对话,并可轻量询问是否需要推荐; 当用户明确说出菜系或菜品(如越南河粉/美式汉堡/Kopi-C)时填写food_intent(cuisines与dishes,并按明确程度给confidence,明确则≥0.6),未提及则food_intent留空; 仅当用户明确说出时提取命名实体(类似food_intent): 电影提取演员(actors)与导演(directors), 音乐提取歌手(artist)与曲风(genres,如rock/edm/classical), 书籍提取作者(author)与出版社(publisher), 商品提取具体商品(product)、类别(category)、品牌(brand)、型号/版本(model)、用途(use_case)、预算(budget与budget_range), 未提及则留空
{profile_context}
回复使用中文"""
        else:
...
JSON格式:
{{"intent":"query|chat", "reply":"回复", "confidence":0.0-1.0, "preferences":{{"domain":"restaurant|movie|music|book|product", "query":"用户原始请求", "genres":["science fiction"], "mood":"relaxing", "tags":["award-winning"], "actors":["Cillian Murphy"], "directors":["Christopher Nolan"], "artist":"Daft Punk", "author":"Brandon Sanderson", "publisher":"Tor", "product":"iPhone", "category":"smartphone", "brand":"Apple", "model":"iPhone 14-16", "use_case":"iOS testing", "budget":"<= 1600 SGD", "restaurant_types":["casual","fine-dining","fast-casual","street-food","buffet","cafe"], "flavor_profiles":["spicy","savory","sweet","sour","mild"], "dining_purpose":"date-night|family|friends|business|solo|celebration", "budget_range":{{"min":20,"max":60,"currency":"SGD"}}, "location":"Chinatown", "food_intent":{{"cuisines":["vietnamese"]或[], "dishes":["pho"]或[], "confidence":0.0-1.0}}}}, "profile_updates":{{"demographics":{{}}, "dining_habits":{{}}}}}}

规则: 仅当用户明确提出想要推荐/查找时才标记为"query"; 普通闲聊/问候/感谢默认"chat"; 只填写用户明确表达或上下文强支持的preferences; 非餐厅请求不要填餐厅默认值; profile_updates可选,仅推断新信息时提供,严格遵循字段规则; 当intent为"chat"时可轻量询问是否需要推荐; 当用户明确说出菜系或菜品(如越南河粉/美式汉堡/Kopi-C)时填写food_intent(cuisines与dishes,并按明确程度给confidence,明确则≥0.6),未提及则food_intent留空; 仅当用户明确说出时提取命名实体(类似food_intent): 电影提取演员(actors)与导演(directors), 音乐提取歌手(artist)与曲风(genres,如rock/edm/classical), 书籍提取作者(author)与出版社(publisher), 商品提取具体商品(product)、类别(category)、品牌(brand)、型号/版本(model)、用途(use_case)、预算(budget与budget_range), 未提及则留空
{profile_context}
回复使用中文"""
        else:
